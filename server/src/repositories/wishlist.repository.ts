...
export class WishlistRepository extends BaseRepository<IWishlist> {
  ...
  async findByUser(userId: string) {
    return Wishlist.findOne({ user: userId }).populate('items.product', 'name sku images price discountPrice stock avgRating weaveType badge careInstructions attributes').exec();
  }
